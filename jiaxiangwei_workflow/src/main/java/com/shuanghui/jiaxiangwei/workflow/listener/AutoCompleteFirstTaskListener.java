...
public class AutoCompleteFirstTaskListener extends DefaultTaskListener {
    ...
    public void onCreate(DelegateTask delegateTask)throws Exception{
        ...
        PvmActivity targerActivity=pvmTransition.getDestination();

        if(!"userTask".equals(targerActivity.getProperty("type"))){
            logger.debug("first activity is not userTask,just skip");
            return;
        }

        if (targerActivity.getId().equals(delegateTask.getExecution().getCurrentActivityId())){
